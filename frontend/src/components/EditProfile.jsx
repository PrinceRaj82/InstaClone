...
const EditProfile = () => {
  ...
  const editProfileHandler = async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.post('https://instaclone-nn56.onrender.com/api/v1/user/profile/edit', formData, {
        headers: {
      ...
      <section className="flex flex-col gap-6 w-full my-8">
        ...
        <div className="flex items-center justify-between bg-gray-100 dark:bg-[#1f1f1f] rounded-xl p-4 bg-[#1f1f1f] text-white">
          <div className="flex items-center gap-3 bg-[#1f1f1f] text-white">
            ...
            <div>
              <h1 className="font-bold text-sm dark:text-white">{user?.username}</h1>
        ...
        <div>
          <h1 className="font-bold text-lg mb-2 dark:text-white">Bio</h1>
          <Textarea
            value={input.bio}
        ...
        <div className="flex justify-end">
          {loading ? (
            <Button className="w-fit bg-[#0095F6] hover:bg-[#2a8ccd] text-white">
              <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              Please wait
            </Button>
          ) : (
            <Button onClick={editProfileHandler} className="w-fit bg-[#0095F6] hover:bg-[#2a8ccd] text-white">
              Submit
            </Button>
          )}
        </div>
